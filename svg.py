...
# DIAGONAL_HATCH ="""<pattern id="diagonalHatch" patternUnits="userSpaceOnUse" width="4" height="4"><path d="M-1,1 l2,-2 M0,4 l4,-4 M3,5 l2,-2" style="stroke:black; stroke-width:1" /></pattern>"""
DIAGONAL_HATCH ="""<pattern id="diagonalHatch" patternUnits="userSpaceOnUse" width="5" height="5"><path d="M-1,1 l2,-2 M0,5 l5,-5 M4,6 l2,-2" style="stroke:black; stroke-width:1" /></pattern>"""
...
def _text(content, x, y, width, height):
    t = ET.Element("text", {
        "x": str(x + width // 2),
        "y": str(y + height // 2),
        "font-size": str(max(5, int(min(width, height) * 0.6))),
...
def board(board=None, *, squares=None, flipped=False, coordinates=True, lastmove=None, check=None, arrows=(), size=None, style=None):
    ...
    .. image:: ../docs/Ne4.svg
    """
    margin = 30 if coordinates else 0
    ...
    defs.append(ET.fromstring(DIAGONAL_HATCH))
